	...
	_In_ int       nCmdShow)
{
	...
	while (true)
	{
		...
		else
		{
			...
			//// Transform만 적용한다. -> 좌표 이동을 위한 함수
			//g_pD2DRenderTarget->SetTransform(D2D1::Matrix3x2F::Translation(0.0f, 500.0f));
			//g_pD2DRenderTarget->DrawBitmap(g_pD2DBitmap);

			//// Rotate만 적용한다.
			//float xp = g_pD2DBitmap->GetSize().width / 2.0f;
			//float yp = g_pD2DBitmap->GetSize().height / 2.0f;

			//g_pD2DRenderTarget->SetTransform(D2D1::Matrix3x2F::Rotation(10.0f, D2D1::Point2F(xp, yp)));
			//g_pD2DRenderTarget->DrawBitmap(g_pD2DBitmap);

			////Scale만 적용한다.          
			//g_pD2DRenderTarget->SetTransform(D2D1::Matrix3x2F::Scale(0.5f, 0.5f, D2D1::Point2F(0.0f, 0.0f)));
			//g_pD2DRenderTarget->DrawBitmap(g_pD2DBitmap);
